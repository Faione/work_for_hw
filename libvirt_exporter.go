...
var (
	...
	libvirtDomainCpuTimeDesc = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "domain_cpu", "time_total"),
		"Amount of CPU time used by the domain, in microseconds.",
		[]string{"domain"},
		nil)
	libvirtDomainCpuUserTimeDesc = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "domain_cpu", "time_user"),
		"Amount of User CPU time used by the domain, in microseconds.",
		[]string{"domain"},
		nil)
	libvirtDomainCpuSysTimeDesc = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "domain_cpu", "time_sys"),
		"Amount of CPU System time used by the domain, in microseconds.",
		[]string{"domain"},
		nil)

	libvirtDomainVcpuTimeDesc = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "domain_vcpu", "time_total"),
		"Amount of CPU time used by the domain's VCPU, in microseconds.",
		[]string{"domain", "vcpu"},
		nil)
	libvirtDomainVcpuDelayDesc = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "domain_vcpu", "delay_total"),
		"Amount of CPU time used by the domain's VCPU, in microseconds. "+
	...
	libvirtDomainVcpuWaitDesc = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "domain_vcpu", "wait_total"),
	...
	libvirtDomainMemoryStatUsedPercentDesc = prometheus.NewDesc(
		...
		[]string{"domain"},
		nil)
	libvirtDomainPerfEventCountDesc = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "domain_perf", "count"),
		"The count of perf event count, that caused by domain.",
		[]string{"domain", "event"},
		nil)
...
func CollectDomain(ch chan<- prometheus.Metric, stat libvirt.DomainStats) error {
	...
	if stat.Cpu.SystemSet {
		ch <- prometheus.MustNewConstMetric(
			libvirtDomainCpuSysTimeDesc,
			prometheus.CounterValue,
			float64(stat.Cpu.System)/1000/1000,
			domainName)
	}

	if stat.Cpu.UserSet {
		ch <- prometheus.MustNewConstMetric(
			libvirtDomainCpuUserTimeDesc,
			prometheus.CounterValue,
			float64(stat.Cpu.User)/1000/1000,
			domainName)
	}
	...
	if stat.Cpu.TimeSet {
		ch <- prometheus.MustNewConstMetric(
			...
			prometheus.CounterValue,
			float64(stat.Cpu.Time)/1000/1000,
			domainName)
	}
	...
	if err != nil {
		...
		for _, vcpu := range domainStatsVcpu {
			...
			ch <- prometheus.MustNewConstMetric(
				...
				prometheus.CounterValue,
				float64(vcpu.CpuTime)/1000/1000, // From nsec to sec
				domainName,
				strconv.FormatInt(int64(vcpu.Number), 10))
	...
	if stat.Perf != nil {
		if stat.Perf.CmtSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.Cmt),
				domainName, "cmt")
		}

		if stat.Perf.MbmtSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.Mbmt),
				domainName, "mbmt")
		}

		if stat.Perf.MbmlSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.Mbml),
				domainName, "mbml")
		}

		if stat.Perf.CpuCyclesSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.CpuCycles),
				domainName, "cpu_cycles")
		}

		if stat.Perf.InstructionsSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.Instructions),
				domainName, "instructions")
		}

		if stat.Perf.CacheReferencesSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.CacheReferences),
				domainName, "cache_references")
		}

		if stat.Perf.CacheMissesSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.CacheMisses),
				domainName, "cache_misses")
		}

		if stat.Perf.BranchInstructionsSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.BranchInstructions),
				domainName, "branch_instructions")
		}

		if stat.Perf.BranchMissesSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.BranchMisses),
				domainName, "branch_misses")
		}

		if stat.Perf.BusCyclesSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.BusCycles),
				domainName, "bus_cycles")
		}

		if stat.Perf.StalledCyclesFrontendSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.StalledCyclesFrontend),
				domainName, "stalled_cycles_frontend")
		}

		if stat.Perf.StalledCyclesBackendSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.StalledCyclesBackend),
				domainName, "stalled_cycles_backend")
		}

		if stat.Perf.RefCpuCyclesSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.RefCpuCycles),
				domainName, "ref_cpu_cycles")
		}

		if stat.Perf.CpuClockSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.CpuClock),
				domainName, "cpu_clock")
		}

		if stat.Perf.TaskClockSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.TaskClock),
				domainName, "task_clock")
		}

		if stat.Perf.PageFaultsSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.PageFaults),
				domainName, "page_faults")
		}

		if stat.Perf.ContextSwitchesSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.ContextSwitches),
				domainName, "context_switch")
		}

		if stat.Perf.CpuMigrationsSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.CpuMigrations),
				domainName, "cpu_migrations")
		}

		if stat.Perf.PageFaultsMinSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.PageFaultsMin),
				domainName, "page_faults_min")
		}

		if stat.Perf.PageFaultsMajSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.PageFaultsMaj),
				domainName, "page_faults_maj")
		}

		if stat.Perf.AlignmentFaultsSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.AlignmentFaults),
				domainName, "alignment_faults")
		}

		if stat.Perf.EmulationFaultsSet {
			ch <- prometheus.MustNewConstMetric(
				libvirtDomainPerfEventCountDesc,
				prometheus.GaugeValue,
				float64(stat.Perf.EmulationFaults),
				domainName, "emulation_faults")
		}
	}
